...
define(function() {
    ...
    else if (engine[2]) useOnload = false; // webkit
    else if (engine[4]) useImportLoad = parseInt(engine[4]) < 18; // gecko
    ...
    var importLoad = function(url, callback) {
        ...
        if (curSheet && curSheet.addImport) { // old IE
            if (ieCurCallback) ieLoads.push([url, callback]);
            else {
                createIeLoad(url);
